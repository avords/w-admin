...
public class HeaderCell implements Cell {
    public String getExportDisplay(TableModel model, Column column) {
        return getI18nTitle(model, column);
    }
    ...
    protected void buildHeaderHtml(HtmlBuilder html, TableModel model, Column column, String headerClass, String sortImage, String sortOrder) {
        ...
        //I18N
        String title = getI18nTitle(model, column);
        ...
        if (column.isSortable()) {
            ...
            if (showTooltips) {
                ...
                html.title(headercellTooltip + " " + title);
            }
        }
	...
	private String getI18nTitle(TableModel model, Column column) {
		String title = column.getTitle();
        if(title!=null){
        	//title = title.toLowerCase().replace(" ", "");
        }
        title = MessageUtils.getMessage(title,LocaleUtils.getLocale(model.getContext()));
		return title;
	}
}
