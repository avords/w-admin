...
<div>
		...
		<jdf:form bean="request" scope="request">
			<form action="${dynamicDomain}/product/productCompanyExclusiveTemplate?ajax=1" method="post"
	...
	<div>
		...
		sortRowsCallback="limit" action="productCompanyExclusiveTemplate?ajax=1">
			...
			<jdf:row>
				<jdf:column property="0" title="<input type='checkbox' class='noBorder' name='pchk' onclick='pchkClick()'/>"
					...
					<input type="checkbox" class="noBorder" name="schk" onclick="schkClick()" value="${currentRowObject.objectId}-${currentRowObject.productNo}-${currentRowObject.name}-${currentRowObject.category.firstName}-${currentRowObject.category.secondName}-${currentRowObject.category.name}-${currentRowObject.sellPrice}" />
				</jdf:column>
	...
	function setProduct(){
		...
		for(var i in products){
			...
			var j = parseInt(i) + 1;
			productDiv.html(productDiv.html()+'<tr id="${inputName}'+count+'"><th><a href="javascript:del(\'${inputName}'+count+'\')" class="btn btn-primary">删除</a></th><th>'+'<input type="hidden" class="search-form-control" name="productCompanyExclusiveForm.${inputName}['+count+'].objectId" value="'+product[0]+'"/>'+j+'</th><th>供应商名称</th>'+
					'<th>'+product[1]+'</th><th>'+product[2]+'</th><th>'+product[3]+'-'+product[4]+'-'+product[5]+'</th><th>'+product[6]+'</th></tr>');
		    count = count+1;
		}
