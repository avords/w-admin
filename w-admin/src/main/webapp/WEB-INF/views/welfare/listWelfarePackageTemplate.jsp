...
<div>
		...
		<jdf:form bean="request" scope="request">
			<form action="${dynamicDomain}/welfarePackage/welfarePackageTemplate?ajax=1" method="post"
	...
	<div>
		...
		sortRowsCallback="limit" action="welfarePackageTemplate?ajax=1">
			...
			<jdf:row>
				<jdf:column property="objectId" title="<input type='checkbox' class='noBorder' name='pchk' onclick='pchkClick()'/>"
							...
							<input type="checkbox" class="noBorder" name="schk" onclick="schkClick()" value="${currentRowObject.objectId}-${currentRowObject.itemName}-${currentRowObject.packageNo}-${currentRowObject.packageName}-<jdf:columnValue dictionaryId="1606" value="${currentRowObject.stockType}" />-<jdf:columnValue dictionaryId="1602" value="${currentRowObject.wpCategoryType}" />" />
				</jdf:column>
	...
	function setPackage(){
		...
		for(var i in products){
			...
			productDiv.html(productDiv.html()+'<tr id="${inputName}'+count+'"><th><a href="javascript:del(\'${inputName}'+count+'\')" class="btn btn-primary">删除</a></th><th>'+'<input type="hidden" class="search-form-control" name="productShieldForm.${inputName}['+count+'].objectId" value="'+product[0]+'"/>'+(count+1)+
					'</th><th>'+product[1]+'-'+product[2]+'-'+product[3]+'</th><th>'+product[4]+'</th><th>'+product[5]+'</th><th>'+product[6]+'</th></tr>');
		count = count+1;
		}
